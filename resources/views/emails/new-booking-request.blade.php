  ...
  <body style="margin:0; background:#f3f3f3; font-family:Arial, Helvetica, sans-serif; color:#111111; line-height:1.7;">
    ...
    <div style="max-width:680px; margin:24px auto; background:#ffffff; border:1px solid #e8e8e8; border-radius:12px; overflow:hidden;">
      ...
      <div style="padding:28px 32px 36px;">
        ...
        <h1 style="margin:0 0 16px; font-size:28px; line-height:1.25; color:#111111;">New Booking Request</h1>
        <p style="margin:0 0 16px; font-size:16px; line-height:1.6; color:#111111;">
          A new booking request has been received and is ready for review.
        </p>
        ...
        <table style="width:100%; border-collapse:collapse; font-size:14px; line-height:1.7;">
          <tr>
            ...
            <td style="padding:8px 0; color:#333333;">{{ $booking->name }}</td>
          </tr>
          <tr>
            <td style="padding:8px 0; font-weight:700; color:#111111;">Email</td>
            <td style="padding:8px 0; color:#333333;">{{ $booking->email }}</td>
          </tr>
          <tr>
            <td style="padding:8px 0; font-weight:700; color:#111111;">Phone</td>
            <td style="padding:8px 0; color:#333333;">{{ $booking->phone }}</td>
          </tr>
          <tr>
            <td style="padding:8px 0; font-weight:700; color:#111111;">Company</td>
            <td style="padding:8px 0; color:#333333;">{{ $booking->company ?? 'N/A' }}</td>
          </tr>
          <tr>
            <td style="padding:8px 0; font-weight:700; color:#111111;">Service</td>
            <td style="padding:8px 0; color:#333333;">{{ $booking->service }}</td>
          </tr>
          <tr>
            <td style="padding:8px 0; font-weight:700; color:#111111;">Scheduled</td>
            <td style="padding:8px 0; color:#333333;">{{ $booking->starts_at ? $booking->starts_at->setTimezone($booking->timezone ?: config('app.timezone'))->format('F j, Y g:i A') : 'Not set' }}</td>
          </tr>
          <tr>
            <td style="padding:8px 0; font-weight:700; color:#111111;">Timezone</td>
            <td style="padding:8px 0; color:#333333;">{{ $booking->timezone_label ?? $booking->timezone ?? 'N/A' }}</td>
          </tr>
        </table>
        ...
        <div style="margin-top:20px; background:#f5f5f5; padding:16px; border-radius:10px; border:1px solid #e9e9e9;">
          ...
          <div style="white-space:pre-wrap; color:#222222; line-height:1.7;">{{ $booking->message ?: 'No additional message' }}</div>
        </div>

        <div style="margin-top:24px; text-align:center;">
          <a href="{{ route('admin.boards') }}" style="display:inline-block; background:#111111; color:#ffffff; text-decoration:none; padding:12px 22px; border-radius:6px; font-weight:700;">View bookings</a>
        </div>
      </div>
    </div>
  </body>
