...
@section('content')
  ...
  <div class="projects-grid" id="videoGrid">
    @if (session('status'))
      <div class="feedback-form-wrap" data-flash-status="{{ session('status') }}" style="display: none; grid-column: 1 / -1; margin-bottom: 1rem;"></div>
    @endif
      ...
      <article class="project-card video-card admin-video-card" data-id="{{ $video->id }}">
        <div class="project-thumb video-thumb hue-{{ ($loop->iteration % 4) + 1 }} {{ $video->video_url ? 'has-video' : '' }}">
          ...
          @if ($video->is_featured)
            <span class="featured-star" title="Featured" aria-label="Featured"></span>
          @endif
        </div>
      </article>
    @empty
    ...
    <div class="new-video-modal" role="dialog" aria-modal="true">
      ...
      <form id="newVideoForm" action="{{ route('admin.projects.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="new-video-field">
          ...
          <p class="upload-hint" style="margin: 6px 0 0; font-size: 12px; color: #6b7280;">MP4, MOV, or WebM — no size limit</p>
        </div>

        <div id="uploadProgress" style="display: none; margin: 4px 0 14px;">
          <div style="height: 6px; border-radius: 999px; background: #e5e7eb; overflow: hidden;">
            <div id="uploadProgressFill" style="height: 100%; width: 0%; background: #4f46e5; transition: width 120ms ease;"></div>
          </div>
          <p id="uploadProgressLabel" style="margin: 6px 0 0; font-size: 12px; color: #6b7280;"></p>
        </div>
        ...
        <div class="field-row">
          ...
          <div class="new-video-field">
            ...
            <label class="toggle-row" for="isFeatured">
              <input id="isFeatured" type="checkbox" name="is_featured" value="1">
