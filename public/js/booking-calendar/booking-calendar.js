...
if (bookingForm) {
    var hasValidationErrors = bookingForm.querySelector(".booking-alert--error") !== null;
    if (!hasValidationErrors) {
        bookingForm.reset();
        if (hiddenSlot) hiddenSlot.value = "";
        if (timezoneField) timezoneField.value = selectedTimezone || "UTC";
        if (timezoneLabelField) timezoneLabelField.value = timezoneOptions.find(opt => opt.getAttribute("aria-selected") === "true")?.dataset.displayLabel || "UTC";
        if (modal) modal.classList.remove("is-open");
        document.body.style.overflow = "";
        timeButtons.forEach(function (btn) {
            btn.classList.remove("time-slot--active", "time-slot--booked", "time-slot--past");
            btn.disabled = false;
            var badge = btn.querySelector(".time-slot__badge");
            if (badge) {
                badge.textContent = "";
                badge.style.display = "none";
            }
