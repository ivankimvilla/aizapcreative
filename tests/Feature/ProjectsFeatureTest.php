...
it('allows multi-gigabyte video uploads without a server-side size cap', function () {
    $this->actingAs(User::factory()->create());

    $tempVideo = tempnam(sys_get_temp_dir(), 'video');
    file_put_contents($tempVideo, 'fake video content');

    $response = $this->post('/admin/projects', [
        'title' => 'Large Upload Video',
        'client' => 'Huge Studio',
        'category' => 'explainer-videos',
        'feature_category' => 'explainer-videos',
        'video_file' => new UploadedFile($tempVideo, 'large-video.mp4', 'video/mp4', 3 * 1024 * 1024 * 1024, true),
        'cover_image' => null,
    ]);
...
it('shows a singular success message when deleting one video', function () {
    $this->actingAs(User::factory()->create());

    $video = ProjectVideo::create([
        'title' => 'Delete Me',
    ...
    $response = $this->delete('/admin/projects', ['ids' => [$video->id]]);
